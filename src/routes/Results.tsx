...
export function Results() {
  ...
  const durationStr = formatDuration(attempt.durationSeconds);
      ...
      <header className="card p-6 grid grid-cols-1 sm:grid-cols-[auto_1fr] gap-6 items-center">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className={`pill text-white ${gradeColor}`}>Grade {grade}</span>
          ...
          <p className="text-neutral-600 dark:text-neutral-400 mt-1">
            {attempt.exam} · {attempt.subject} · Time used {durationStr}
          </p>
          <div className="flex flex-wrap gap-2 mt-4">
            <button
              type="button"
              className="btn-primary"
              onClick={() => navigate(`/practice/${attempt.exam}/${encodeURIComponent(attempt.subject)}`)}
            >
              Retry this test
            </button>
            <button
            ...
            topicStats.map((s) => (
              <div key={s.topic} className="p-4">
                ...
                <div className="h-2 rounded-full bg-neutral-200 dark:bg-neutral-800 overflow-hidden">
                  <div
                    className={`h-full ${s.pct >= 70 ? 'bg-emerald-500' : s.pct >= 50 ? 'bg-amber-400' : 'bg-red-500'}`}
                    style={{ width: `${s.pct}%` }}
                  />
                </div>
              </div>
