...
import { countBySource, listQuestionsForSubject, listSubjects } from '../data/loader';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {subjects.map((subject) => {
          ...
          const source = countBySource(exam, subject.name);
          return (
            <Link
              key={subject.id}
            ...
            >
              ...
              <div className="mt-2 text-xs text-neutral-500 dark:text-neutral-400 flex items-center gap-2 flex-wrap">
                <span className="pill bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300">
                  {questionCount} {questionCount === 1 ? 'question' : 'questions'}
                </span>
                {source.generated > 0 && source.real > 0 ? (
                  <>
                    <span
                      className="pill bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300"
                      title="Past exam questions"
                    >
                      {source.real} real
                    </span>
                    <span
                      className="pill bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300"
