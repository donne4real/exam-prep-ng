...
- **9 routes**: Home, Subjects, Year Picker, Test Runner, Results, Dashboard, Flashcards, Flashcard Deck, Languages
- **Real past questions only** — every question carries a source label (see below)
- **Year-based practice** — pick a specific exam year or a mixed set; thin
  years transparently fall back to a mixed set with an on-screen notice
- **Lazy per-subject loading** — the app boots from a ~4KB index; a subject's
  questions download only when you open that subject
...
- **Offline support** via Workbox service worker (cache-first for assets,
  stale-while-revalidate for all data files)
- **Dark mode** — auto / light / dark, persisted in `localStorage`
- **Progress tracking** — attempts, average score, weak topics (syllabus-tagged), day streak
- **Timed tests** with auto-submit on expiry
- **Friendly empty state** when data is not yet available
...
```bash
# 1. Rebuild the data files from the real extracted sources
...
Output layout (small first load, per-subject offline caching):

```
public/data/
├── index.json            # ~4KB: exams + subject list + counts + years
└── banks/
    ├── bece-mathematics.json
    ├── jamb-english-language.json
    └── ...               # one file per exam+subject
```
...
no duplicate questions within an exam+subject, no cross-exam duplicates, and
— for the split layout — that every subject's declared counts, years, and
topics match its bank file exactly.

Questions are tagged with syllabus topics (e.g. Mensuration, Comprehension,
Constitution & Rule of Law) via keyword rules in `build_bank.py`. The
tagging is heuristic — teacher review will refine it over time — and
untagged questions are simply omitted from topic breakdowns.
...
- `data/index.json` + `data/banks/*.json`: `StaleWhileRevalidate` (cached
  files serve instantly — essential offline — and refresh in the background
