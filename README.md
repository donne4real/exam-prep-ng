...
└── src/
    ...
    │   └── exam.ts
└── scripts/
    ├── knowledge-base.js         # curated fill-in-the-blank facts
    ├── generate-questions.mjs    # runs the generation pipeline
    ├── validate-questions.mjs    # sanity-checks the JSON
    └── add-explanations.mjs      # one-off helper (used during KB setup)
```
...
The repo includes a generation pipeline for filling question-bank gaps. It
is **not run automatically**; you invoke it manually when the knowledge
base has new facts or when the per-subject target needs adjusting.

```bash
node scripts/generate-questions.mjs   # generate up to 150 per exam-subject
node scripts/validate-questions.mjs  # sanity-check the resulting JSON
```

- `scripts/knowledge-base.js` — curated fill-in-the-blank facts per
  subject. To add a subject, push a `{prompt, options, correct, topic, explanation}` array under a `'{EXAM}|{SUBJECT}'` key.
- `scripts/generate-questions.mjs` — produces up to 12 distinct questions
  per fact (option order × correct-position permutations), then dedups
  against the existing bank.
- Generated questions are tagged `source: 'generated'` in the JSON and the
  UI surfaces a "practice" badge so students can distinguish them from
  real past-exam questions.

The provenance tracking flows through the entire stack: each `Question` has
a `source` field, `AttemptAnswer` snapshots it at submit time, and the
Results page shows a real-vs-practice breakdown for every attempt.
